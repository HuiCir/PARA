...
class HF_VLLM(BaseLM):
    ...
    def load_model(self):
        self.llm = LLM(model=self.llm_path, tokenizer=self.llm_path, dtype=self.dtype)#, tensor_parallel_size = 2)
        #self.llm = torch.nn.DataParallel(llm, device_ids=list(range(2)))
        ...
        if self.generation_stop != '':
            self.sampling_params = SamplingParams(temperature=self.temperature, top_p=self.top_p, 
                                                ...
                                                repetition_penalty= 1, max_tokens = self.generate_maxlength, logprobs=5 ,skip_special_tokens = False)
        else:
            self.sampling_params = SamplingParams(temperature=self.temperature, top_p=self.top_p, 
    ...
    def generate(self, inputs: Union[str,list[str]], sampling_params = None)->list[BaseLM.Outputs]:
        ...
        for RequestOutput in outputs:
            ...
            #print(RequestOutput)
            Outputs.tokens_logprob = []
            x = [logprob[token_id] for token_id, logprob in zip(Outputs.tokens_ids, RequestOutput.outputs[0].logprobs)]
            for log in x:
                Outputs.tokens_logprob.append(log.logprob)
